...
function initializeState() {
  ...
  if (!existingProducts || existingProducts.includes("Café Gourmet") || existingProducts.includes('"id":"p1"') || !existingProducts.includes("stock")) {
...
// Synchronize state with Flask backend when online
async function syncWithBackend() {
  const user = getCurrentUser();
  if (!user) return; // user already logged out, do nothing

  // 1. Fetch products (no auth required — safe to always call)
  if (typeof apiGetProducts === "function") {
    try {
      ...
      console.warn("apiGetProducts failed:", e.message);
      throw e;
    }
  }

  // Check again after first await — a 401 in products could have cleared the user
  if (!getCurrentUser()) return;

  // 2. Fetch telemetry (no auth required — safe to always call)
  if (typeof apiGetTelemetry === "function") {
    try {
      const telemetry = await apiGetTelemetry();
      if (telemetry && telemetry.status) {
        const localTelemetry = getTelemetry();
        const mappedTelemetry = {
          battery: telemetry.battery,
          status: telemetry.status,
          currentOrderId: telemetry.current_order_id,
          speed: telemetry.speed,
          distance: telemetry.distance,
          eta: telemetry.eta,
          startTime: telemetry.start_time,
          deliveryEndTime: telemetry.delivery_end_time,
          returnEndTime: telemetry.return_end_time,
          history: telemetry.history || localTelemetry.history
        };
        saveTelemetry(mappedTelemetry);
      }
    } catch (e) {
      console.warn("apiGetTelemetry failed:", e.message);
      throw e;
    }
  }

  // Check again — 401 from telemetry could have cleared the user
  if (!getCurrentUser()) return;

  // 3. Fetch orders (requires auth — null return means 401 was handled)
  if (typeof apiGetOrders === "function") {
    try {
      const orders = await apiGetOrders();
      // null means 401 was received and handled by apiFetch (user cleared)
      ...
      if (orders && Array.isArray(orders)) {
        const mappedOrders = orders.map(o => ({
          ...
          timing: o.timing,
          notes: o.notes,
          summaryText: o.summary_text,
          productName: o.summary_text
        }));
        saveOrders(mappedOrders);
      }
    } catch (e) {
      console.warn("apiGetOrders failed:", e.message);
      throw e;
    }
  }
}
