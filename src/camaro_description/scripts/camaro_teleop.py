...
pygame.display.set_caption("SMART CAMARO - Teleoperador")
...
def desenhar_tecla(tela, letra, x, y, pressionada):
    cor = AMARELO_CAMARO if pressionada else (50, 50, 50)
...
def desenhar_botao_aceleracao(tela, ativo):
    cor = AMARELO_CAMARO if ativo else (90, 90, 90)
    # fundo levemente preenchido quando ativo, para reforçar o estado "ligado"
    if ativo:
        pygame.draw.rect(tela, (45, 35, 0), (ACCEL_BTN_X, ACCEL_BTN_Y, ACCEL_BTN_W, ACCEL_BTN_H), border_radius=6)
    else:
...
while True:
    ...
    for linha in linhas:
        ...
        pygame.draw.line(tela, AMARELO_CAMARO, (linha['x'], linha['y']), (linha['x'] + linha['length'], linha['y']), 2)

    # Títulos
    titulo = fonte_titulo.render("SMART CAMARO", True, AMARELO_CAMARO)
    subtitulo = fonte_sub.render("CONTROLADOR DE VELOCIDADE - GAZEBO HARMONIC", True, AMARELO_CAMARO)
    ...
    pygame.draw.rect(tela, AMARELO_CAMARO, (barra_x, 140, largura_barra, 18), border_radius=6)
    ...
    pygame.draw.circle(tela, AMARELO_CAMARO, (joy_x, joy_y), 24)
    ...
    pygame.draw.rect(tela, AMARELO_CAMARO if editando_topico else (80, 80, 90),
    ...
    # Entrada
    cor_modo = AMARELO_CAMARO if modo == "JOYSTICK" else (200, 200, 0)
    ...
    pygame.draw.rect(tela, AMARELO_CAMARO, (btn_x, 15, 135, 36), border_radius=8, width=3)
    txt_vel = fonte_desc.render(f"▶ {config['label']}", True, AMARELO_CAMARO)
